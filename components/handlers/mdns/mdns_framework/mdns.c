...
static const char *TAG = "MDNS DRIVER";
...
static esp_err_t _mdns_service_task_start(void)
{
    ...
    if (!_mdns_service_task_handle) {
        xTaskCreatePinnedToCore(_mdns_service_task, "mdns", MDNS_SERVICE_STACK_DEPTH, NULL, MDNS_TASK_PRIORITY,
                                (TaskHandle_t *const)(&_mdns_service_task_handle), MDNS_CORE_AFFINITY);
        if (!_mdns_service_task_handle) {
            _mdns_stop_timer();
            MDNS_SERVICE_UNLOCK();
...
esp_err_t mdnsFramework_init(void)
{
    esp_err_t err = ESP_OK;
...
void start_mdns_service(char* hostname)
{
    ...
    esp_err_t err = mdnsFramework_init();
    if (err) {
